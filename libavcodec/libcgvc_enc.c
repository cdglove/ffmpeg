...
static av_cold int cgvc_enc_init(AVCodecContext *c)
{
    // struct cgvc_enc_context *cgvc = c->priv_data;
    // int err;
...
static av_cold int cgvc_enc_close(AVCodecContext *c)
{
    // struct cgvc_enc_context *cgvc = c->priv_data;
...
static int cgvc_enc_encode(AVCodecContext *avctx, AVPacket *avpkt,
...
AVCodec ff_cgvc_encoder = {
    .name           = "cgvc",
    .long_name      = NULL_IF_CONFIG_SMALL("Chris Glover Video Codec"),
    .type           = AVMEDIA_TYPE_VIDEO,
    .id             = AV_CODEC_ID_CGVC,
    .priv_data_size = 0, //sizeof(struct cgvc_enc_context),
    .init           = cgvc_enc_init,
    .close          = cgvc_enc_close,
    .encode2        = cgvc_enc_encode,
